...
- 持久化用户信息和旅行偏好。
- AI 行程生成支持流式返回。
- 大模型接入采用 OpenAI-compatible 协议，供应商通过配置切换。
- 内置本地 MCP 风格工具，保证 Demo 稳定可跑。
- 支持外部 MCP Server 适配层，用于天气、搜索、景点、地图和路线工具。
...
默认实现采用 OpenAI-compatible 协议。供应商切换通过配置完成：
...
本地工具提供确定性数据，保证演示稳定。外部 MCP 工具通过配置启用，不影响行程规划主流程代码。
...
- 外部 MCP 工具失败时，如果已配置本地工具，则切换本地工具。
- 路线数据不可用时，使用本地默认交通时间估算，并标记为估算值。
